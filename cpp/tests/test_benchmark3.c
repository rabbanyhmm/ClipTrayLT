...
static AtspiRect* find_caret_smart(AtspiAccessible* obj, int depth) {
    ...
    gboolean is_active = atspi_state_set_contains(states, ATSPI_STATE_ACTIVE);
    gboolean is_showing = atspi_state_set_contains(states, ATSPI_STATE_SHOWING);
    g_object_unref(states);

    // If neither active nor focused, and not showing, don't descend deep
    if (depth > 1 && !is_active && !is_focused && !is_showing) {
        return NULL;
    }
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (child) {
            AtspiRect* r = find_caret_smart(child, depth + 1);
...
int main() {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (app) {
            AtspiStateSet* s = atspi_accessible_get_state_set(app);
            gboolean active_or_focused = s && (atspi_state_set_contains(s, ATSPI_STATE_ACTIVE) ||
                                              atspi_state_set_contains(s, ATSPI_STATE_FOCUSED));
            if (s) g_object_unref(s);

            if (active_or_focused) {
                r = find_caret_smart(app, 0);
                g_object_unref(app);
                if (r) break;
            }
            g_object_unref(app);
    ...
    if (r) {
        printf("Smart Caret lookup: %.2f ms! X=%d, Y=%d\n", elapsed_ms, r->x, r->y);
        g_free(r);
    } else {
        printf("No caret found (%.2f ms)\n", elapsed_ms);
    }
    return 0;
}
